...
Usage:
    ...
    python live_trader.py --universe ZC=F ZS=F ZW=F
...
class LiveTrader:
    def __init__(self, universe, initial_cash, strategy_name,
        ...
        log.info("Loading yield/weather data...")
        self.yield_model.load_data()

        log.info("Training yield model...")
        self.yield_model.train_all()

        log.info("Running live macro forecast...")
        ...
        log.info(
            f"Yield forecast: "
            f"{self.yield_signal['mid']:.2f} "
            f"(revision={self.yield_signal.get('yield_revision', 0):+.2f})"
        )
    ...
    def tick(self, print_report_flag: bool = False):
        ...
        for ticker, model in self._models.items():
            ...
            ctx = SignalContext(
                ...
                yield_revision=self.yield_signal.get(
                    "yield_revision",
                    0
                ),

                drought_index=self.yield_signal.get(
                    "drought_index",
                    0
                ),
            )
            ...
            log.info(
                ...
                f"yield_rev={self.yield_signal.get('yield_revision', 0):+.2f} | "
                f"drought={self.yield_signal.get('drought_index', 0):.2f} | "
