...
def _make_portfolio(tickers=None, cash=100_000):
    return Portfolio(tickers or ["AAPL"], initial_cash=cash, fee_schedule=FEES_ZERO)


def test_initial_nav():
    pf = _make_portfolio()
    assert pf._compute_nav({"AAPL": 150.0}) == 100_000.0


def test_buy_increases_position():
    pf  = _make_portfolio()
    ts  = datetime(2024, 1, 2)
    pf.step(ts, {"AAPL": 150.0}, {"AAPL": Signal.BUY})
    assert pf.positions["AAPL"].shares > 0
...
def test_sell_closes_position():
    ...
    ts = datetime(2024, 1, 2)
    pf.step(ts, {"AAPL": 150.0}, {"AAPL": Signal.BUY})
    pf.step(datetime(2024, 1, 3), {"AAPL": 160.0}, {"AAPL": Signal.SELL})
    assert pf.positions["AAPL"].shares == 0.0


def test_fees_reduce_cash():
    pf_free = _make_portfolio(cash=100_000)
    pf_paid = Portfolio(["AAPL"], initial_cash=100_000, fee_schedule=FEES_RETAIL)
    ts = datetime(2024, 1, 2)
    pf_free.step(ts, {"AAPL": 150.0}, {"AAPL": Signal.BUY})
    pf_paid.step(ts, {"AAPL": 150.0}, {"AAPL": Signal.BUY})
...
def test_stop_loss_closes_position():
    pf = _make_portfolio()
    ts = datetime(2024, 1, 2)
    pf.step(ts, {"AAPL": 100.0}, {"AAPL": Signal.BUY})
    # Price drops 10% — exceeds 5% stop-loss
    pf.step(datetime(2024, 1, 3), {"AAPL": 90.0}, {"AAPL": Signal.HOLD})
    assert pf.positions["AAPL"].shares == 0.0


def test_multi_asset():
    pf = _make_portfolio(["AAPL", "MSFT"])
    ts = datetime(2024, 1, 2)
    pf.step(ts, {"AAPL": 150.0, "MSFT": 300.0},
            {"AAPL": Signal.BUY, "MSFT": Signal.BUY})
    assert pf.positions["AAPL"].shares > 0
    assert pf.positions["MSFT"].shares > 0


def test_report_keys():
    pf = _make_portfolio()
    pf.step(datetime(2024, 1, 2), {"AAPL": 150.0}, {"AAPL": Signal.BUY})
    r = pf.report({"AAPL": 155.0})
